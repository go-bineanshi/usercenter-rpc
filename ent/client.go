...
import (
	...
	"github.com/go-bineanshi/usercenter-rpc/ent/migrate"
...
// UpdateOneID returns an update builder for the given id.
func (c *UserAuthClient) UpdateOneID(id uint64) *UserAuthUpdateOne {
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *UserAuthClient) DeleteOneID(id uint64) *UserAuthDeleteOne {
...
func (c *UserAuthClient) Get(ctx context.Context, id uint64) (*UserAuth, error) {
	return c.Query().Where(userauth.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *UserAuthClient) GetX(ctx context.Context, id uint64) *UserAuth {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
type (
	hooks struct {
		User, UserAuth []ent.Hook
	}
	inters struct {
		User, UserAuth []ent.Interceptor
	}
)
